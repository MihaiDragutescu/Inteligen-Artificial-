...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == Joc.JMAX:
        ...
        for mutare in stare.mutari_posibile:
            stare_actualizata = alpha_beta(alpha, beta, mutare)
            if scor_curent < stare_actualizata.scor:
                scor_curent = stare_actualizata.scor
                stare.stare_aleasa = mutare
                if alpha < stare_actualizata.scor:
                    alpha = stare_actualizata.scor
                    if alpha >= beta:
                        break
                        # am produs alpha-retezarea

    elif stare.j_curent == Joc.JMIN:
        # Daca joaca MIN, scor curent ia o valoare foarte mare pentru inceput, apoi, mai jos, se actualizeaza cu scorul cel
        # mai avantajos al fiilor
        scor_curent = 9999
        ...
        for mutare in stare.mutari_posibile:
            ...
            if scor_curent > stare_actualizata.scor:
                scor_curent = stare_actualizata.scor
                stare.stare_aleasa = mutare
                if beta > stare_actualizata.scor:
                    beta = stare_actualizata.scor
                    if alpha >= beta:
                        break
                        # am produs beta-retezarea
...
def main():
    ...
    while True:
        optiune=int(input('Alegeti 1 pt MinMax Retezare sau 2 pentru AlphaBeta Retezare: '))
